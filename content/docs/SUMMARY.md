# Table of contents

* [Kerb](README.md)

## Concepts

* [Address poisoning](concepts/address-poisoning.md)
* [The asymmetry](concepts/the-asymmetry.md)
* [The dwell window](concepts/dwell-window.md)
* [The trust list](concepts/trust-list.md)
* [Threat model](concepts/threat-model.md)

## Protocol

* [Overview](protocol/overview.md)
* [Transfer lifecycle](protocol/lifecycle.md)
* [State](protocol/state.md)
* [Parameters](protocol/parameters.md)
* [Events and errors](protocol/events-and-errors.md)

## Implementation

* [KerbCore](implementation/core.md)
* [KerbLens](implementation/lens.md)
* [Assets](implementation/assets.md)
* [Security](implementation/security.md)
* [Testing](implementation/testing.md)

## Integration

* [Frontend](integration/frontend.md)

## Reference

* [Glossary](reference/glossary.md)
* [FAQ](reference/faq.md)
* [Honest limits](reference/limits.md)
